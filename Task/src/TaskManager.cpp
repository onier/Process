...
TaskGroup::TaskGroup(const std::vector<std::string> &tasks) : _tasks(tasks) {}

std::shared_ptr<Process::AbstractTask> TaskManager::getTaskByID(std::string id) {
    for (auto &t: _tasks) {
        if (t->getID() == id) {
            return t;
        }
...
void TaskManager::createElement(rttr::instance obj2, xercesc::DOMElement *domElement, xercesc::DOMDocument *document,
...
bool TaskManager::saveDomElement(xercesc::DOMElement *domElement, std::shared_ptr<xercesc::DOMDocument> document) {
        ...
        if (name == "tasks") {
            ...
            for (int i = 0; i < tasks->getLength(); ++i) {
                ...
                if (taskType.is_valid()) {
                    ...
                    auto absPtr = taskVariant.get_value<std::shared_ptr<Process::AbstractTask>>();
...
std::string TaskManager::saveXML() {
...
bool TaskManager::loadXML(std::string value) {
    xercesc::XMLPlatformUtils::Initialize();
    ...
    std::shared_ptr<xercesc::MemBufInputSource> memBufIS = std::shared_ptr<xercesc::MemBufInputSource>(
            new xercesc::MemBufInputSource((const XMLByte *) value.data(), value.size(), ""));
