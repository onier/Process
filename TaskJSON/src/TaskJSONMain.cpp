...
void edit(std::shared_ptr<rttr::variant> variant) {
    auto pro = variant->get_type().get_property("Text");
    std::string text = "123";
    pro.set_value(*variant, text);
    LOG(INFO) << pro.get_value(*variant).to_string();
}

int main() {
    ExclusiveRule ee;
    rttr::variant exclusiveRule = rttr::type::get_by_name("ExclusiveRule").create();
    std::shared_ptr<rttr::variant> varPtr = std::make_shared<rttr::variant>(exclusiveRule);
//    auto pro = exclusiveRule.get_type().get_property("Text");
//    std::string text ="123";
//    pro.set_value(exclusiveRule,text);
//    LOG(INFO)<<pro.get_value(exclusiveRule).to_string();
    edit(varPtr);
    auto pro = exclusiveRule.get_type().get_property("Text");
    LOG(INFO) << pro.get_value(*varPtr).to_string();
    auto rule = (*varPtr).get_value<ExclusiveRule>();
    LOG(INFO)<<rule._text;
//    LOG(INFO)<<exclusiveRule<ExclusiveRule>()._text;
    FakeTask fakeTask;
...
//    process.loadXML(text);
    FakeTask fakeTask1;
    process.loadFile("/home/ubuntu/puppy/Process/TaskJSON/src/demo.xml");
    process.initProcessValues({{"taskValue", 2.0}});
    //    folly::Synchronized<std::map<std::string, boost::any>> values;
    //    values.wlock()->insert({"taskValue", 2.0});
    process.startProcess(nullptr);
